...
class DrillingOptimizer:
    ...
    # Sigma bounds (to prevent explosion/collapse)
    sigma_max: float = 1.0
    sigma_min: float = 1e-8
    initial_sigma_stored: float = 0.1  # For reference
    ...
    def __init__(self, start_x: np.ndarray, start_y: float, initial_sigma: float = 0.1, bounds: Optional[List[Tuple[float, float]]] = None):
        ...
        # FIX Finding 26: Sanitize start_x - replace NaN/Inf with bounds center
        start_x = np.array(start_x, dtype=float)
        if not np.all(np.isfinite(start_x)):
            if bounds is not None:
                centers = np.array([(lo + hi) / 2 for lo, hi in bounds])
            else:
                centers = np.full(self.dim, 0.5)  # Default [0,1] assumption
            invalid_mask = ~np.isfinite(start_x)
            start_x[invalid_mask] = centers[invalid_mask]
        
        self.mu = start_x.copy()
        
        # FIX Finding 26: Sanitize start_y
        # For minimization: NaN → +Inf (any value will be an improvement)
        # +Inf is kept as-is (any value will be an improvement)
        # -Inf is kept as-is (nothing can improve, drilling will stagnate)
        if np.isnan(start_y):
        ...
        self.initial_sigma_stored = self.sigma
        
        # Compute sigma bounds based on problem scale
        if bounds is not None:
            widths = [hi - lo for lo, hi in bounds]
            self.sigma_max = np.mean(widths)  # Max = average bounds width
        else:
            self.sigma_max = 1.0
        self.sigma_min = 1e-8
    ...
    def tell(self, x: np.ndarray, y: float) -> bool:
        ...
        if success:
            ...
            # Update sigma with capping
            # Simple 1/5th-inspired rule with bounded growth
            ...
            # Reduced from +10 to +2 to avoid monopolizing global budget
        ...
        if self.sigma <= self.sigma_min:
            return False  # Converged (sigma collapsed)
            
        if self.stagnation_counter > 10:
            # Moderate patience (10 failures) before giving up
            return False

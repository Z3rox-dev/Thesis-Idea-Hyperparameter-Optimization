...
class AdaptiveConfig:
    ...
    def adapt(self, dim: int, budget: int):
        ...
        # 3. Tree Depth
        # Logic: Log2(dim) is standard for spatial partitioning trees (k-d trees etc)
        # Base depth 5 + log2(dim)
        self.max_depth = 5 + int(math.log2(dim))
...
class QuadCube:
    ...
    def ucb(self, beta: float = 1.6, eps: float = 1e-8, lambda_geo: float = 0.0, vol_scale: float = 1.0, value_scale: float = 1.0) -> float:
        ...
        bonus = lambda_eff * vol_rel * value_scale / decay
        ucb_val = base + bonus
    ...
    def should_split(self,
        ...
        if remaining_budget is not None:
            ...
            # RELAXED: Allow splitting if we can cover at least 2 children fully (or 4 partially).
            init_cost = 2 * min_points
        ...
        # Lowered threshold to min_points + 4 to encourage depth with limited budget.
        # For min_points=8, this splits at 12 instead of 16.
        if n_trials_now >= min_points + 4:
            self.split_block_reason = None
            return split_type
        # ---------------------------------------------------
...
class QuadHPO:
    def __init__(
        ...
        self.debug_log_path = os.path.join(log_dir, f"quadhpo_adaptive_debug_{timestamp}.log")
    ...
    def _pick_best_child(self, children: List[QuadCube]) -> QuadCube:
        ...
        return max(children, key=lambda c: c.ucb(beta=beta_def, lambda_geo=lambda_def, vol_scale=root_vol, value_scale=global_sigma))

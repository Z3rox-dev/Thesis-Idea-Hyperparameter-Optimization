...
class QuadCube:
    ...
    def fit_surrogate(self, min_points: Optional[int] = None) -> None: #Prendo i dati che ho già misurato, li guardo da un’angolazione comoda (PCA) e ci disegno sopra una parabola (tipo “colline” e “valli”) che approssima la zona vicina.
        ...
        # Prendiamo i punti locali
        pairs = getattr(self, "_tested_pairs", [])

        # Se ne abbiamo pochi, chiediamo al genitore
        if len(pairs) < min_points * 2 and self.parent is not None:
            ...
            if len(parent_pairs) > 0:
                if len(pairs) == 0:
                    extra = list(parent_pairs)
                else:
                    # Simple deduplication by checking if point is close to any existing point
                    # This is O(N*M) but N, M are small (< 50)
            ...
            if needed > 0:
                # DEBUG: Log borrowing
                # print(f"[DEBUG] Cube {id(self)} borrowing {min(len(extra), needed)} points from parent")
    ...
    def ucb(self, beta: float = 1.6, eps: float = 1e-8, lambda_geo: float = 0.0) -> float:
        ...
        geo_decay = 1.0 / (1.0 + 0.2 * self.depth)
        lambda_eff = lambda_geo * geo_decay
        
        bonus = lambda_eff * vol / decay
        ucb_val = base + bonus
    ...
    def should_split(self,
                     ...
                     gate_ratio: float = 0.2) -> str: #decide se e come dividere il cubo
        ...
        # Se la cella è molto peggio del best globale, non splittare.
        ...
        if self.surrogate_2d is not None and self.surrogate_2d.get('n', 0) > 15:
            r2 = self.surrogate_2d.get('r2', 0.0)
            # Se il modello spiega bene i dati (> 0.85), NON SPLITTARE.
            if r2 > 0.85:
                self.split_block_reason = f"good_model_fit (r2={r2:.3f} > 0.85)"
                return 'none'
        # ----------------------------------------------------------------
...
class QuadHPO:
    def __init__(
        ...
        # Meta-optimizable parameters
        min_trials: int = 7,
        gamma: float = 0.0858,
        delta_prune: float = 0.7206,
        split_gate_ratio: float = 0.5294,
        min_points: int = 8,
        debug_log: bool = True,
    ) -> None:
        # ============================================================
        ...
        self.lambda_geo = float(lambda_geo)  # Geometric shrinkage rate
        ...
        # ============================================================
        self.min_trials = int(min_trials)           # Minimum samples before split
        self.gamma = float(gamma)             # UCB exploration coefficient
        self.stale_steps_max = 15     # Max iterations without improvement before prune
        self.delta_prune = float(delta_prune)        # Prune threshold
        self.max_depth = 6             # Maximum tree depth (optimized: kept at 6)
        self.min_width = 1e-4         # Minimum cube width (optimized: was 1e-3)
        self.min_points = int(min_points)           # Minimum points for surrogate
        self.min_leaves = 5           # Minimum leaf count
        self.prune_grace_period = 3   # Min trials before a leaf can be pruned
        self.split_gate_ratio = float(split_gate_ratio)   # Global best gating ratio
        ...
        self.gauss_scale = 0.4        # Optimized: Increased from 0.16 to 0.4 for better coverage
        ...
        self.debug_log_path = os.path.join(log_dir, f"quadhpo_new_debug_{timestamp}.log")
        ...
        self.root.surrogate_min_points = self.min_points
    ...
    def _sample_biased_in(self, cube: QuadCube, alpha: float = 0.4, top_k: int = 5) -> np.ndarray:
        ...
        if pairs and np.random.rand() < alpha:
            ...
            x = np.clip(x, 0.0, 1.0)
            self._debug_log(f"[Trial {self.trial_id}] Sampling: Biased Random")
            return x
        ...
        self._debug_log(f"[Trial {self.trial_id}] Sampling: Geometric Fallback")
    ...
    def select_cube(self) -> QuadCube:
        ...
        if self.trial_id % 5 == 0: # Log every 5 trials
            ...
            for i, (c, val) in enumerate(candidates[:5]):
                ...
                geo_decay = 1.0 / (1.0 + 0.2 * c.depth)
                lambda_eff = self.lambda_geo * geo_decay
                bonus = lambda_eff * vol / decay
                base = val - bonus
    ...
    def run_trial(self, cube: QuadCube, objective_fn: Callable[[np.ndarray, int], Any]) -> None:
        ...
        ucb_val = cube.ucb(beta=self.beta, lambda_geo=self.lambda_geo)
        # Calculate effective beta for logging
        depth_decay = 1.0 / (1.0 + 0.1 * cube.depth)
        beta_eff = self.beta * depth_decay
        self._debug_log(f"[Trial {self.trial_id}] Selected Cube {id(cube)}: depth={cube.depth}, n={cube.n_trials}, "
                       f"mean={cube.mean_score:.4f}, var={cube.var_score:.4f}, ucb={ucb_val:.4f}, beta_eff={beta_eff:.5f}")

        x_norm = self._sample_biased_in(cube, alpha=0.2, top_k=5)
    ...
    # No per-trial debug prints in production
        ...
        split_type = cube.should_split(
            ...
            gate_ratio=self.split_gate_ratio,
        )

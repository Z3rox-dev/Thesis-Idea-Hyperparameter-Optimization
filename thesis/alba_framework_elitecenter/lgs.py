...
def fit_lgs_model(
    ...
    if len(pairs) >= dim + 3:
        ...
        try:
            ...
            if grad_norm > 1e-9:
                lgs_gradient_dir = grad / grad_norm
            else:
                lgs_gradient_dir = None
            
            # IMPROVEMENT: Compute elite-center direction (more robust on RF surfaces)
            # Direction from center towards elite points
            elite_center = np.mean(top_k_pts, axis=0)
            elite_dir_raw = elite_center - center
            elite_dir_raw = elite_dir_raw / widths  # Normalize by cube widths
            elite_dir_norm = np.linalg.norm(elite_dir_raw)
            
            if elite_dir_norm > 1e-9:
                elite_gradient_dir = elite_dir_raw / elite_dir_norm
            else:
                elite_gradient_dir = None
            
            # BLEND: 60% elite direction + 40% LGS gradient
            # Elite direction is more robust on piecewise-constant surfaces
            alpha = 0.4  # LGS weight
            if lgs_gradient_dir is not None and elite_gradient_dir is not None:
                combined = alpha * lgs_gradient_dir + (1 - alpha) * elite_gradient_dir
                combined_norm = np.linalg.norm(combined)
                if combined_norm > 1e-9:
                    gradient_dir = combined / combined_norm
                else:
                    gradient_dir = elite_gradient_dir
            elif elite_gradient_dir is not None:
                gradient_dir = elite_gradient_dir
            elif lgs_gradient_dir is not None:
                gradient_dir = lgs_gradient_dir
        except Exception:
            pass

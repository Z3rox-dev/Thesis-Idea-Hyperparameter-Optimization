#!/usr/bin/env python3
"""
ALBA V3 - Fast Start Adaptive Local Bayesian Algorithm

Key insight: ALBA V1 è lento nelle prime 25-50 iterazioni.
Questa versione ottimizza il warm-up per partire velocemente.

Improvements:
1. Latin Hypercube Sampling per inizializzazione (migliore copertura) - 3% budget ottimale
2. Warm-up: LHS fornisce copertura uniforme iniziale per stimare gamma correttamente
3. Previene split prematuri con dati rumorosi

Component Value Analysis (cifar10 ablation):
- Cube partitioning: +2.7% ★★★
- CatThompson:      +2.4% ★★★
- Local search:     +2.5% ★★★
- LHS warm-up:      +1.0% ★★
- LGS model:        +0.8% ★
...
class ALBA:
    """
    ALBA V3 - Fast Start version.
    
    Key difference: Better initialization and faster warm-up.
    """
    ...
    def __init__(
        ...
        # V3-specific: warm-up parameters
        warm_up_fraction: float = 0.03,  # Fraction of budget for LHS warm-up (3% optimal)
        early_split_factor: float = 0.5,  # Split earlier during warm-up
    ) -> None:
        self.bounds = bounds
        self.dim = len(bounds)
        ...
        # V3 parameters
        self._warm_up_fraction = warm_up_fraction
        self._early_split_factor = early_split_factor
        self._warm_up_budget = max(5, int(total_budget * warm_up_fraction))
        
        # Pre-generate LHS samples for warm-up
        self._lhs_samples = latin_hypercube_sample(self._warm_up_budget, self.dim, self.rng)
        self._lhs_index = 0
        
        # Apply bounds to LHS samples
        for i in range(self.dim):
            lo, hi = self.bounds[i]
            self._lhs_samples[:, i] = lo + self._lhs_samples[:, i] * (hi - lo)
        
        # Track top-k points from warm-up for exploitation
        self._warmup_top_k = []  # Will store (x, y) tuples
        self._n_warmup_exploitation = 3  # Number of top warm-up points to exploit
    ...
    def _in_warm_up_phase(self) -> bool:
        return self.iteration < self._warm_up_budget

    def ask(self) -> np.ndarray:
        self.iteration = len(self.X_all)

        # V3: LHS warm-up phase
        if self._in_warm_up_phase():
    ...
    def _apply_categorical_sampling_warmup(self, x: np.ndarray) -> np.ndarray:
        """Discretize categorical dimensions during warm-up (uniform sampling)."""
        if not self.categorical_dims:
            return x
        
        x = x.copy()
        for dim_idx, n_choices in self.categorical_dims:
            # Discretize to nearest categorical value
    ...
    def tell(self, x: np.ndarray, y_raw: float) -> None:
        ...
        if self.last_cube is not None:
            ...
            # V3: Skip model fitting during warm-up for speed
            if not self._in_warm_up_phase():
                cube.fit_lgs_model(self.gamma, self.dim, self.rng)

            # V3: More aggressive splitting during/after warm-up
    ...
    def _sample_in_cube(self, cube: Cube) -> np.ndarray:
        # V3: After warm-up, use standard sampling
    ...
    def _should_split(self, cube: Cube) -> bool:
        ...
        # V3: Lower threshold right after warm-up for fast tree growth
        just_after_warmup = self._warm_up_budget <= self.iteration < self._warm_up_budget + 20
        if just_after_warmup:
            factor = self._split_trials_factor * self._early_split_factor
        else:
            factor = self._split_trials_factor
        
        return cube.n_trials >= factor * self.dim + self._split_trials_offset
    ...
    def optimize(self, objective: Callable[[np.ndarray], float], budget: int = 100) -> Tuple[np.ndarray, float]:
        if budget != self.total_budget:
            ...
            # Re-generate LHS if budget changed
            self._warm_up_budget = max(5, int(budget * self._warm_up_fraction))
            self._lhs_samples = latin_hypercube_sample(self._warm_up_budget, self.dim, self.rng)
            self._lhs_index = 0
            for i in range(self.dim):
                lo, hi = self.bounds[i]

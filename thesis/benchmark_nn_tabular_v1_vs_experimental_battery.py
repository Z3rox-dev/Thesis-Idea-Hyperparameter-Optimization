#!/usr/bin/env python3
"""HPOBench NN-Tabular battery: ALBA_V1 (param_space) vs ALBA_V1_experimental.
...
Example:
  ...
  python thesis/benchmark_nn_tabular_v1_vs_experimental_battery.py --budget 400 --seeds 70-79
"""
...
def main() -> int:
    p = argparse.ArgumentParser(description="NN Tabular battery: ALBA_V1(param_space) vs ALBA_V1_experimental")
    ...
    out_path = f"{RESULTS_DIR}/nn_tabular_v1_vs_experimental_b{args.budget}_s{seeds[0]}-{seeds[-1]}_{timestamp}.json"
    ...
    for task_id in task_ids:
        ...
        # param_space for V1: each hp is categorical sequence
        param_space = {name: hp_seqs[name] for name in hp_names}

        results["tasks"].setdefault(str(task_id), {"v1": {}, "experimental": {}, "errors": {}})
        save_results(out_path, results)
        ...
        for seed in seeds:
            ...
            try:
                ...
                for it in range(args.budget):
                    cfg = opt_v1.ask()  # dict, already typed
                ...
                results["tasks"][str(task_id)]["v1"][str(seed)] = cp
                save_results(out_path, results)
                ...
                v1_final = results["tasks"][str(task_id)]["v1"][str(seed)].get(str(args.budget))
                ...
                print(f"task {task_id} seed {seed}: V1={v1_final:.6f} EXP={exp_final:.6f} -> {winner} ({elapsed:.1f}s)")

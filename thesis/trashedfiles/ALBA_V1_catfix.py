#!/usr/bin/env python3
"""
ALBA_V1_catfix - Version with fix for categorical exploration/exploitation.

The key issue: ALBA converges too quickly on suboptimal categorical values.
In the JAHS benchmark, ALBA found N=3 (local optimum ~91.56%) instead of N=5 (global ~91.71%).

Fixes:
1. Global categorical tracking independent of cube splitting
2. Thompson sampling for categorical exploration (sample from posterior)
3. Periodic categorical exploration bursts
4. Ensure all categorical values get minimum exploration
"""
...
class ALBA:
    """ALBA with categorical exploration fix."""
    ...
    def __init__(
        ...
        cat_exploration_prob: float = 0.15,  # NEW: probability of exploring different cat
        cat_min_samples: int = 10,  # NEW: minimum samples per cat value
    ) -> None:
        self.bounds = bounds
        self.dim = len(bounds)
        ...
        self.cat_exploration_prob = cat_exploration_prob
        self.cat_min_samples = cat_min_samples
        ...
        # GLOBAL categorical stats (independent of cubes)
        # {dim_idx: {val_idx: (sum_scores, n_samples)}}
        self.global_cat_stats: Dict[int, Dict[int, Tuple[float, int]]] = {}
        for dim_idx, n_choices in self.categorical_dims:
            self.global_cat_stats[dim_idx] = {v: (0.0, 0) for v in range(n_choices)}
        
        # Track best score per categorical value
        # {dim_idx: {val_idx: best_score}}
        self.best_per_cat: Dict[int, Dict[int, float]] = {}
        for dim_idx, n_choices in self.categorical_dims:
            self.best_per_cat[dim_idx] = {v: -np.inf for v in range(n_choices)}
    ...
    def ask(self) -> np.ndarray:
        ...
        if self.iteration < self.exploration_budget:
            ...
            # FIX: Apply global categorical exploration
            x = self._apply_global_cat_exploration(x)
    ...
    def tell(self, x: np.ndarray, y_raw: float) -> None:
        ...
        for dim_idx, n_choices in self.categorical_dims:
            ...
            sum_scores, n = self.global_cat_stats[dim_idx][val_idx]
            self.global_cat_stats[dim_idx][val_idx] = (sum_scores + y, n + 1)
            
            if y > self.best_per_cat[dim_idx][val_idx]:
                self.best_per_cat[dim_idx][val_idx] = y
    ...
    def _apply_global_cat_exploration(self, x: np.ndarray) -> np.ndarray:
        """FIX: Use global categorical stats with Thompson sampling for exploration."""
        ...
        for dim_idx, n_choices in self.categorical_dims:
            # Check if any categorical value needs more exploration
            min_samples = min(self.global_cat_stats[dim_idx][v][1] for v in range(n_choices))
            
            # If under-explored, force exploration
            if min_samples < self.cat_min_samples:
                under_explored = [v for v in range(n_choices) 
                                if self.global_cat_stats[dim_idx][v][1] < self.cat_min_samples]
                if self.rng.random() < 0.3:  # 30% chance to explore under-sampled
                    chosen = self.rng.choice(under_explored)
                    x[dim_idx] = self._cat_value_to_continuous(chosen, n_choices)
                    continue
            
            # Thompson sampling: sample from posterior for each cat value
            if self.rng.random() < self.cat_exploration_prob:
                # Use Thompson sampling
                samples = []
                for v in range(n_choices):
                    sum_scores, n = self.global_cat_stats[dim_idx][v]
                    if n > 0:
                        mean = sum_scores / n
                        # Posterior variance inversely prop to n
                        std = 1.0 / np.sqrt(n + 1)
                        sample = self.rng.normal(mean, std)
                    else:
                        # Optimistic prior for unexplored
                        sample = self.rng.normal(0, 2.0)  # High variance
                    samples.append(sample)
                
                chosen = int(np.argmax(samples))
                x[dim_idx] = self._cat_value_to_continuous(chosen, n_choices)
            # else: keep the value from cube-based sampling

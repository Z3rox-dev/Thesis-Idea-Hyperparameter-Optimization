#!/usr/bin/env python3
"""
ALBA_V1_catfix2 - Version 2 of categorical fix with more conservative approach.

The issue with catfix was too much exploration variance.
This version:
1. Uses UCB-like approach for categoricals instead of Thompson sampling
2. Only explores under-sampled categories initially
3. Reduces exploration over time (annealing)
4. Better integration with existing good_ratio logic
"""
...
class ALBA:
    """ALBA with conservative categorical UCB fix."""
    ...
    def __init__(
        ...
        # GLOBAL categorical stats for UCB
        # {dim_idx: {val_idx: (sum_scores, n_samples, best_score)}}
        self.global_cat_stats: Dict[int, Dict[int, Tuple[float, int, float]]] = {}
        for dim_idx, n_choices in self.categorical_dims:
            self.global_cat_stats[dim_idx] = {v: (0.0, 0, -np.inf) for v in range(n_choices)}
    ...
    def ask(self) -> np.ndarray:
        ...
        if self.iteration < self.exploration_budget:
            ...
            # Apply global UCB categorical exploration
            x = self._apply_cat_ucb(x)
    ...
    def tell(self, x: np.ndarray, y_raw: float) -> None:
        ...
        for dim_idx, n_choices in self.categorical_dims:
            ...
            sum_scores, n, best = self.global_cat_stats[dim_idx][val_idx]
            self.global_cat_stats[dim_idx][val_idx] = (
                sum_scores + y, 
                n + 1, 
                max(best, y)
            )
    ...
    def _apply_cat_ucb(self, x: np.ndarray) -> np.ndarray:
        """Apply UCB-based categorical exploration/exploitation."""
        if not self.categorical_dims:
            return x
        
        x = x.copy()
        total_n = len(self.X_all) + 1
        
        # Annealing: explore more early, exploit more later
        progress = self.iteration / max(1, self.exploration_budget)
        explore_weight = 2.0 * (1 - progress * 0.7)  # Starts at 2.0, ends at ~0.6
        
        for dim_idx, n_choices in self.categorical_dims:
            # Only apply UCB sometimes (50% when early, 20% when late)
            apply_ucb_prob = 0.5 * (1 - progress * 0.6)
            if self.rng.random() > apply_ucb_prob:
                continue  # Keep cube-based sampling
            
            # Compute UCB for each categorical value
            ucb_scores = []
            for v in range(n_choices):
                sum_scores, n, best = self.global_cat_stats[dim_idx][v]
                ...
                if n == 0:
                    ...
                    mean = sum_scores / n
                    # UCB bonus: sqrt(2 * ln(total_n) / n)
                    exploration_bonus = explore_weight * np.sqrt(2 * np.log(total_n + 1) / n)
                    ucb = mean + exploration_bonus
                
                ucb_scores.append(ucb)
            
            # Softmax selection based on UCB scores
            ucb_arr = np.array(ucb_scores)
            # Temperature decreases over time (more greedy later)
            temp = 5.0 * (1 - progress * 0.7) + 0.5
            ucb_arr = ucb_arr - ucb_arr.max()  # Numerical stability
            probs = np.exp(ucb_arr * temp)

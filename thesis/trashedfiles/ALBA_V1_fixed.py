#!/usr/bin/env python3
"""
ALBA_V1_fixed - Version with structural fixes based on debug analysis.

Key fixes:
1. Better leaf selection: prioritize leaves containing the best point
2. More aggressive local refinement around best when stuck
3. Smarter split strategy: don't split if best is in cube  
4. Dynamic exploration bonus based on cube quality
5. Better categorical handling with stronger exploitation
"""
...
class Cube:
    ...
    contains_global_best: bool = False  # FIX: track if contains global best
...
class ALBA:
    ...
    def __init__(
        ...
        # FIX: Track best cube
        self._best_cube: Optional[Cube] = None
    ...
    def _update_best_cube_flags(self):
        """FIX: Mark which cube contains the global best."""
        for leaf in self.leaves:
            leaf.contains_global_best = False
        
        if self.best_x is not None:
            for leaf in self.leaves:
                if leaf.contains(self.best_x):
                    leaf.contains_global_best = True
                    self._best_cube = leaf
                    break
    ...
    def ask(self) -> np.ndarray:
        ...
        if self.iteration < self.exploration_budget:
            self._update_gamma()
            self._recount_good()
            self._update_best_cube_flags()

            if self.iteration % 5 == 0:
                self._update_all_models()

            # FIX: If stagnating heavily, do aggressive local search
            if self.stagnation > self._stagnation_threshold * 2 and self.best_x is not None:
                # 50% chance of local search around best when very stuck
                if self.rng.random() < 0.5:
                    x = self._intensified_local_search()
                    self.last_cube = self._find_containing_leaf(x)
                    return x
    ...
    def tell(self, x: np.ndarray, y_raw: float) -> None:
        ...
        if self.last_cube is not None:
            ...
            # FIX: Don't split if cube contains global best and is reasonably small
    ...
    def _select_leaf(self) -> Cube:
        """FIX: Better leaf selection with best-cube bonus."""
        if not self.leaves:
            return self.root
        ...
        for c in self.leaves:
            ...
            # FIX: Strong bonus for cube containing global best
            best_bonus = 0.0
            if c.contains_global_best:
                # Adaptive bonus: higher when not stagnating, to exploit
                # Lower when stagnating to explore elsewhere
                if self.stagnation < self._stagnation_threshold:
                    best_bonus = 0.3
                else:
                    best_bonus = 0.1  # Still some bonus but encourage exploration
            
            scores.append(ratio + exploration + model_bonus + best_bonus)
    ...
    def _intensified_local_search(self) -> np.ndarray:
        """FIX: More aggressive local search when very stuck."""
        if self.best_x is None:
            return np.array([self.rng.uniform(lo, hi) for lo, hi in self.bounds])
        
        # Use smaller radius for intensification
        radius = 0.05 + 0.05 * self.rng.random()
        noise = self.rng.normal(0, radius, self.dim) * self.global_widths
        x = self.best_x + noise
        
        # Apply categorical sampling with strong exploitation
        x = self._apply_categorical_sampling_exploit(x)
    ...
    def _generate_candidates(self, cube: Cube, n: int) -> List[np.ndarray]:
        ...
        # FIX: If cube contains best, include more perturbations around best
        include_best_perturbations = cube.contains_global_best and self.best_x is not None

        for i in range(n):
            strategy = self.rng.random()

            # FIX: Dedicated best perturbation strategy
            if include_best_perturbations and strategy < 0.3:
                # Perturb best point with varying radii
                radius = 0.02 + 0.15 * self.rng.random()
                x = self.best_x + self.rng.normal(0, radius, self.dim) * widths
            elif strategy < 0.45 and model is not None and len(model["top_k_pts"]) > 0:
                idx = self.rng.integers(len(model["top_k_pts"]))
                x = model["top_k_pts"][idx] + self.rng.normal(0, 0.15, self.dim) * widths
            elif strategy < 0.60 and model is not None and model["gradient_dir"] is not None:
    ...
    def _apply_categorical_sampling_exploit(self, x: np.ndarray) -> np.ndarray:
        """FIX: Stronger exploitation for categorical dims during local search."""
        if not self.categorical_dims or self.best_x is None:
            return x
        
        x = x.copy()
        
        # Use global stats to decide
        for dim_idx, n_choices in self.categorical_dims:
            # With high probability, use same categorical as best
            if self.rng.random() < 0.8:
                x[dim_idx] = self.best_x[dim_idx]
            else:
                # Small perturbation
                best_cat = self._discretize_cat(self.best_x[dim_idx], n_choices)
                # Neighbor categories
                neighbors = [best_cat]
                if best_cat > 0:
                    neighbors.append(best_cat - 1)
                if best_cat < n_choices - 1:
                    neighbors.append(best_cat + 1)
                chosen = self.rng.choice(neighbors)
                x[dim_idx] = self._cat_value_to_continuous(chosen, n_choices)
    ...
    def _local_search_sample(self, progress: float) -> np.ndarray:
        ...
        # FIX: Apply categorical exploitation in local search
        x = self._apply_categorical_sampling_exploit(x)
    ...
    def _should_split(self, cube: Cube) -> bool:
        ...
        # FIX: Don't split if cube contains best and is already reasonably small
        if cube.contains_global_best:
            cube_vol = cube.volume()
            total_vol = np.prod(self.global_widths)
            # If cube is less than 10% of total volume, don't split
            if cube_vol < 0.1 * total_vol:
                return False

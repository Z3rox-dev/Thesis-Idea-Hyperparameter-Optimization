...
class Cube:
    ...
    def ucb(self, beta: float, global_best: float = -np.inf, root_vol: float = 1.0) -> float:
        if self.n_trials > 0:
        ...
        # Volume bonus: esplora cubi grandi non ancora visitati (molto leggero)
        d = len(self.bounds)
        vol = float(np.prod(self._widths()))
        vol_factor = (vol / max(root_vol, 1e-12)) ** (1.0/max(d, 1))
        geo_decay = 1.0 / (1.0 + 0.2 * self.depth)
        # Bonus piccolo e solo per cubi poco esplorati
        vol_bonus = 0.05 * vol_factor * geo_decay / math.log(self.n_trials + 2) if self.n_trials < 10 else 0.0
        
        result = float(mu + explore + vol_bonus)
...
class HPOptimizer:
    ...
    def _sample(self, cube: Cube) -> np.ndarray:
        ...
        # Line Search su PC1 (10% prob): esplora lungo asse principale
        if np.random.rand() < 0.10:
            cube._ensure_frame()
            u = np.zeros(d)
            lo1, hi1 = cube.bounds[0]
            u[0] = np.random.uniform(lo1, hi1)  # Esplora tutto PC1
            for j in range(1, d):
                lo, hi = cube.bounds[j]
                wid = abs(hi - lo)
                u[j] = self._trunc_norm(0.0, wid*0.08, lo, hi)  # Stretti sugli altri
            x = cube.to_original(u)
            if self._in_bounds(x):
                self._last_sample_method = 'line_search'
                return self._clip(x)
        
        # Ottimo surrogata RILASSATO: usa anche con R² moderato (0.5+)
        if s and s['n'] >= 2*d+2 and s['r2'] >= 0.5 and s['mode'] == 'quad':
            w = s['w']
            # Calcola ottimo per ogni dimensione (anche se curvatura non tutta negativa)
            t_opt = np.zeros(d)
            valid_opt = True
            for j in range(d):
                if abs(w[d+1+j]) > 1e-9:
                    ...
                    t_opt[j] = 0.0  # Se curvatura piatta, resta al centro
            
            # Se R² >= 0.85 e tutte curvature negative: punto diretto
            all_neg = all(w[d+1+j] < -1e-9 for j in range(d))
            if s['r2'] >= 0.85 and all_neg:
            ...
            # Altrimenti: perturba ATTORNO all'ottimo (sfrutta modello anche se imperfetto)
            widths = cube._widths()
            noise = 0.08 * widths * np.random.randn(d)
            t_pert = t_opt + noise
            # Clip alle bounds del cubo
            for j in range(d):
                lo, hi = cube.bounds[j]
                t_pert[j] = np.clip(t_pert[j], lo, hi)
            x_pert = s['mu'] + s['R'] @ t_pert
            if self._in_bounds(x_pert):
                self._last_sample_method = 'quad_pert'
                return self._clip(x_pert)
    ...
    def _select(self) -> Cube:
        if self._preferred is not None:
            ...
            self._preferred = None
        beta = self._beta()
        # Calcola volume root per bonus geometrico
        root_vol = float(np.prod(self.root._widths()))
        return max(self.leaves, key=lambda c: c.ucb(beta, self.best_score, root_vol))

    def _backprop(self, leaf: Cube, score: float) -> None:
        """Propaga statistiche ai parent per decisioni split più informate."""
        curr = leaf.parent
        while curr:
            curr.n_trials += 1
            n = curr.n_trials
            ...
            # Aggiorna anche la lista _tested_pairs del parent (per surrogate più informato)
            # Nota: NON rifittiamo il surrogate qui per evitare overhead
            curr = curr.parent
    ...
    def _prune(self) -> None:
        ...
        keep = []
        for c in self.leaves:
            if c.n_trials == 0 or c.ucb(beta, self.best_score, root_vol) >= thresh:
                keep.append(c)
        if len(keep) < min_leaves:
            ranked = sorted(self.leaves, key=lambda c: c.ucb(beta, self.best_score, root_vol), reverse=True)
            for c in ranked:
        ...
        for i in range(budget):
            ...
            if i >= init_phase:
                ...
                if mode != 'none':
                    ...
                    root_vol = float(np.prod(self.root._widths()))
                    self._preferred = max(children, key=lambda c: c.ucb(self._beta(), self.best_score, root_vol))
                if self.trial % 5 == 0:
                    self._prune()

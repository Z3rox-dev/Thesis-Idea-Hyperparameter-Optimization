...
class AdaptiveConfig:
    ...
    def adapt(self, dim: int, budget: int):
        ...
        # CHANGE: Require more points before splitting to allow Quadratic fit
        # Linear needs D+1. Diagonal needs 2D+1.
        # We want to allow the node to accumulate enough points for a Diagonal fit (2D+2)
        # before we even consider splitting, to ensure the split decision is informed.
        self.min_points_split = 2 * dim + 2
...
class QuadCube:
    ...
    # Debug logger callback (set by QuadHPO)
    ...
    def fit_surrogate(self, min_points: int, ridge_alpha: float = 1e-3) -> None:
        ...
        # DECISION: Full 2D vs Diagonal ND vs Linear ND
        # Progressive Complexity:
        # 1. Linear (D+2 <= n < 2D+2): Fit plane. Stable with few points.
        # 2. Quadratic (n >= 2D+2): Fit parabola. Needs more points.
        
        n_samples = len(pairs)
        n_required_linear = d + 2
        n_required_quad = 2 * d + 2
        
        if n_samples < n_required_linear:
            self.surrogate_2d = None
            return

        # Always use Diagonal/Linear logic (Legacy Full 2D removed)
        mode = 'quadratic'
        if n_samples < n_required_quad:
            mode = 'linear'
        
        if mode == 'linear':
            # print(f"DEBUG: Activating Linear ND (d={d}, n={n_samples})")
            # Linear Model: [1, t1...tD]
            # Params: 1 + D
            n_params = d + 1
            ...
            # w_final must be full size (2D+1) for compatibility
            w = np.zeros(2 * d + 1)
            w[0] = w_scaled[0]
            w[1:d+1] = w_scaled[1:] / t_std
            # Quadratic terms remain 0.0
            
            # Curvature is zero for linear model
            lam = np.zeros(d)
            
        else:
            # print(f"DEBUG: Activating Diagonal Quadratic ND (d={d}, n={n_samples})")
        ...
        # --- SANITY CHECK 2: Surrogate Predictions ---
        if np.any(np.isnan(y_hat)) or np.any(np.isinf(y_hat)):
            msg = "[CRITICAL] Il surrogato ha predetto NaN/Inf!"
            if hasattr(self, '_debug_logger') and self._debug_logger is not None:
                self._debug_logger(msg)
        elif np.max(y_hat) > 1000 or np.min(y_hat) < -1000:
            msg = f"[WARNING] Predizioni surrogato fuori scala: min={np.min(y_hat):.4f}, max={np.max(y_hat):.4f}"
            if hasattr(self, '_debug_logger') and self._debug_logger is not None:
                self._debug_logger(msg)
        # ---------------------------------------------
        ...
        final_type = 'linear' if mode == 'linear' else 'diag'
    ...
    def predict_surrogate(self, x_prime: np.ndarray) -> Tuple[float, float]:
        ...
        stype = self.surrogate_2d.get('type', 'diag')
        d = len(x_prime)
        ...
        if stype == 'linear':
            # --- LINEAR CASE ---
            # Phi FULL for Mean (y_hat) - w is padded to 2D+1
            Phi = np.zeros(2 * d + 1)
            Phi[0] = 1.0
            Phi[1:d+1] = x_prime
            Phi[d+1:] = x_prime**2 # Even if w quadratic terms are 0, needed for dot product
            
            # Phi SCALED REDUCED for Variance (A_inv is D+1 x D+1)
            Phi_scaled = np.zeros(d + 1)
            Phi_scaled[0] = 1.0
            Phi_scaled[1:] = x_prime_scaled
        ...
        # FIX: Use Predictive Variance (sigma2 * (1 + v)) instead of Mean Variance (sigma2 * v)
        # This prevents variance collapse as N increases, ensuring exploration 
        # when residuals (noise/mismatch) are high.
        var_pred = sigma2 * (1.0 + v)
    ...
    def sample_uniform_prime(self) -> np.ndarray:
        ...
        for i, (lo, hi) in enumerate(self.bounds):
            ...
            # --- FIX: Use Truncated Normal even for Uniform? No, Uniform is Uniform.
            # But we must ensure we respect bounds strictly.
            # np.random.uniform is generally safe, but let's be explicit.
    ...
    def sample_uniform(self) -> np.ndarray:
        ...
        x = self.to_original(x_prime)
        
        # --- FIX: Ensure Uniform respects global bounds ---
        # Rotation can cause slight out-of-bounds due to float precision
        # or if the cube is slightly misaligned.
        # We clamp here to be safe, but it should be rare.
        # Note: We don't have access to global bounds here easily without passing them.
        # But the caller (QuadHPO) will clip anyway.
        return x
    ...
    def update_final(self, final_score: float, x: Optional[np.ndarray] = None, min_points: int = 10, ridge_alpha: float = 1e-3) -> None: #Aggiorna statistiche finali: numero prove, lista degli score, media, varianza
        self.n_trials += 1
        self.scores.append(float(final_score))
        
        if x is not None:
            if not hasattr(self, "_tested_pairs"):
                self._tested_pairs = []
            self._tested_pairs.append((np.array(x, dtype=float), float(final_score)))
            self.add_tested_point(x)
    ...
    def _recalc_stats_from_history(self) -> None:
        """Recalculate n_trials, mean, var, best_score from _tested_pairs."""
        pairs = getattr(self, "_tested_pairs", [])
        if not pairs:
            return
        ...
        if n > 1:
            self.var_score = float(np.var(scores, ddof=1))
            # Reconstruct M2 for Welford consistency (M2 = variance * (n-1))
            self.M2 = self.var_score * (n - 1)
        else:
            self.var_score = 0.0
            self.M2 = 0.0
            
        # Also sync the scores list for leaf_score consistency
    ...
    def should_split(self,
        ...
        # STRICT: Always wait for min_trials in the current node before splitting.
        # Inherited history helps the model, but we need local verification.
        n_trials_now = self.n_trials
        hist_len_now = len(self._points_history)
        
        if n_trials_now < min_trials:
             self.split_block_reason = f"insufficient_trials (n_trials={n_trials_now} < {min_trials})"
             return 'none'
        ...
        if S is not None and float(np.max(S)) < 1e-6:
            # Check if we have a strong gradient (Linear Model)
            # If gradient is strong, we should split even if curvature is zero.
            has_gradient = False
            if self.surrogate_2d is not None and 'w' in self.surrogate_2d:
                w = np.asarray(self.surrogate_2d['w'], dtype=float)
                d = len(self.bounds)
                # w = [bias, lin_1..D, quad...]
                if len(w) >= d + 1:
                    lin_norm = float(np.linalg.norm(w[1:d+1]))
                    if lin_norm > 1e-4:
                        has_gradient = True
        ...
        # Increased threshold to allow model maturation (Diagonal Quad needs 2D+2 points).
        # Force split only when we are clearly hoarding data.
        hoarding_threshold = 3 * min_points
        if n_trials_now >= hoarding_threshold:
            self.split_block_reason = None
            return split_type
        # ---------------------------------------------------
    ...
    def _principal_axes(self,
        ...
        # --- SANITY CHECK 1: PCA Instability ---
        cond_number = evals.max() / (evals.min() + 1e-12)
        if cond_number > 1e10:
            msg = f"[WARNING] PCA Instabile! Cond Number: {cond_number:.2e}\n          Eigvals: {evals}"
            if hasattr(self, '_debug_logger') and self._debug_logger is not None:
                self._debug_logger(msg)
        # ---------------------------------------
        ...
        # --- SANITY CHECK 4: PCA Reconstruction Error ---
        # Proietta sui primi 2 assi principali e ricostruisci
        k_pca = min(2, d)
        U_k = evecs[:, :k_pca]  # shape (d, k_pca)
        Z = (pts - mu) @ U_k   # Proiezione: shape (n, k_pca)
        pts_reconstructed = Z @ U_k.T + mu  # Ricostruzione: shape (n, d)
        reconstruction_errors = np.linalg.norm(pts - pts_reconstructed, axis=1)  # shape (n,)
        mean_recon_error = float(np.mean(reconstruction_errors))
        max_recon_error = float(np.max(reconstruction_errors))
        
        # Normalizza rispetto alla diagonale del bounding box dei punti
        pts_range = np.max(pts, axis=0) - np.min(pts, axis=0)
        diag_length = float(np.linalg.norm(pts_range)) + 1e-12
        relative_error = mean_recon_error / diag_length
        
        if relative_error > 0.5:  # Se perdiamo più del 50% dell'informazione
            msg = (f"[WARNING] PCA Reconstruction Error elevato!\n"
                   f"          Mean Error: {mean_recon_error:.4f}, Max Error: {max_recon_error:.4f}\n"
                   f"          Relative Error (vs diag): {relative_error:.2%}\n"
                   f"          Variance Explained (top-{k_pca}): {np.sum(evals[:k_pca])/np.sum(evals):.2%}")
            if hasattr(self, '_debug_logger') and self._debug_logger is not None:
                self._debug_logger(msg)
        # ------------------------------------------------
        # anisotropy check
    ...
    def _quad_cut_along_axis(self,
        ...
        if c_hat > 1e-8:
            ...
            # DEBUG: Check for frame mismatch
            # print(f"DEBUG_CUT: axis={axis_idx}, t_star={t_star:.4f}, bounds=({lo:.4f}, {hi:.4f})")
            
            t_cut = float(np.clip(t_star, lo, hi))
            if abs(t_cut) < 1e-9:
                 print(f"[DEBUG_ZERO_CUT] axis={axis_idx} t_star={t_star} clipped to {t_cut} in [{lo}, {hi}]")
            return t_cut
        ...
        # DEBUG: Check for frame mismatch in fallback
        # print(f"DEBUG_CUT_MED: axis={axis_idx}, t_med={t_med:.4f}, bounds=({lo:.4f}, {hi:.4f})")
        
        t_cut = float(np.clip(t_med, lo, hi))
        if abs(t_cut) < 1e-9:
             print(f"[DEBUG_ZERO_CUT_MED] axis={axis_idx} t_med={t_med} clipped to {t_cut} in [{lo}, {hi}]")
        return t_cut
    ...
    def split2(self, axis: Optional[int] = None, ridge_alpha: float = 1e-3) -> List["QuadCube"]:
        ...
        self._ensure_frame()
        widths_parent = self._widths()
        # --- FIX: Ensure we always cut in the CURRENT frame (self.R) ---
        # Even if we use PCA/Surrogate to choose the axis, we must map it back to self.R
        # and use self.R for the cut calculation, because children inherit self.R.
        ...
        if axis is not None:
            ...
            if S is not None:
                ...
                # Map surrogate axis to Prime axis
                R_c = self.R if self.R is not None else np.eye(d)
                M = R_s.T @ R_c
                ax = int(np.argmax(np.abs(M[ax_s, :])))
                
            elif ok:
                # Use PCA frame spans to find best PCA axis
                R_pca = R_loc
                M = R_pca.T @ (self.R if self.R is not None else np.eye(d))
                spans_pca = (np.abs(M) @ widths_parent)
                ax_pca = int(np.argmax(spans_pca))
                
                # Map PCA axis to Prime axis
                ax = int(np.argmax(np.abs(M[ax_pca, :])))
            else:
                # Fallback: widest axis in current frame
                ax = int(np.argmax(widths_parent))
        ...
        for (ch, ctr_prime) in ((c1, center_left_prime), (c2, center_right_prime)):
            ...
            ch._tested_points = []
            # Inherit debug logger from parent
            if hasattr(self, '_debug_logger') and self._debug_logger is not None:
        ...
        # --- FIX: Recalculate stats for children to avoid amnesia ---
        c1._recalc_stats_from_history()
        c2._recalc_stats_from_history()
        # ------------------------------------------------------------
    ...
    def split4(self, ridge_alpha: float = 1e-3) -> List["QuadCube"]:
        ...
        if ok:
            cut_i = self._quad_cut_along_axis(ax_i, R_use, mu_use, ridge_alpha=ridge_alpha, bounds=base_bounds[ax_i])
            cut_j = self._quad_cut_along_axis(ax_j, R_use, mu_use, ridge_alpha=ridge_alpha, bounds=base_bounds[ax_j])
        else:
...
class QuadHPO:
    def __init__(
        ...
        self.root._debug_logger = self._debug_log  # Set logger callback
        ...
        self.debug_log_path = os.path.join(log_dir, f"quadhpo_debug_debug_{timestamp}.log")
    ...
    def _is_within_bounds(self, x: np.ndarray) -> bool:
        for j, (lo, hi) in enumerate(self.bounds):
            if x[j] < lo - 1e-9 or x[j] > hi + 1e-9:
                return False
        return True

    def _clip_and_log_candidate(self, x: np.ndarray, source: str) -> np.ndarray:
        d = len(self.bounds)
        x_orig = x.copy()
        clipped = False
        
        # --- SANITY CHECK 3: Clipping Silenzioso ---
        is_clamped = np.zeros(d, dtype=bool)
        for j in range(d):
            lo, hi = self.bounds[j]
            if x[j] <= lo + 1e-9 or x[j] >= hi - 1e-9:
                is_clamped[j] = True
        
        if np.sum(is_clamped) > (d / 2):
            self._debug_log(f"[WARNING] Candidato schiacciato sui bordi! Dims on bounds: {np.sum(is_clamped)}/{d}")
        # -------------------------------------------
        ...
        for j in range(d):
            ...
            if abs(x[j] - val) > 1e-12:
                clipped = True
        
        if clipped or np.all(np.abs(x) < 1e-9):
            pass  # Verbose debug removed
            # self._debug_log(f"[CLIP/ZERO] Source={source} Trial={self.trial_id}")  # Removed: verbose debug
            # self._debug_log(f"  Pre : {x_orig}")  # Removed: verbose debug
            # self._debug_log(f"  Post: {x}")  # Removed: verbose debug
        return x

    def _sample_trunc_norm(self, mean: float, std: float, low: float, high: float) -> float:
        """Sample from a Truncated Normal distribution using fast rejection sampling."""
        if std <= 1e-12:
            return float(np.clip(mean, low, high))
        
        # Fast path: if bounds are wide relative to std, use rejection sampling
        # (much faster than scipy.truncnorm for typical cases)
        for _ in range(100):
            val = np.random.normal(mean, std)
            if low <= val <= high:
                return float(val)
        
        # Fallback to clipping if rejection fails (rare for reasonable std)
    ...
    def _sample_biased_in(self, cube: QuadCube, alpha: float = 0.4, top_k: int = 5) -> np.ndarray:
        ...
        if pairs and np.random.rand() < alpha:
            ...
            # Retry loop for global bounds
            x = None
            for _ in range(10):
                center, _ = pick[np.random.randint(len(pick))]
                
                # --- FIX: Truncated Normal Sampling (Optuna Style) ---
                # Perturb in prime frame relative to bounds using Truncated Normal
                t_center = cube.to_prime(np.array(center, dtype=float))
                t_perturb = np.zeros(d)
                for j in range(d):
                    lo, hi = cube.bounds[j]
                    width = abs(hi - lo)
                    # Perturb by 10% of width
                    scale = max(width * 0.1, 1e-6)
                    
                    # Sample directly inside [lo, hi] centered at t_center[j]
                    val = self._sample_trunc_norm(t_center[j], scale, lo, hi)
                    t_perturb[j] = val - t_center[j]
                
                x = cube.to_original(t_center + t_perturb)
                
                if self._is_within_bounds(x):
                    # self._debug_log(f"[Trial {self.trial_id}] Sampling: Biased Random (TruncNorm)")  # Removed: verbose debug
                    return self._clip_and_log_candidate(x, "BiasedRandom")
            
            # Fallback: Smart Clamp
            # self._debug_log(f"[Trial {self.trial_id}] Sampling: Biased Random (Smart Fallback)")  # Removed: verbose debug
            x_fallback = np.zeros(d)
            for j in range(d):
                lo, hi = self.bounds[j]
                c_valid = float(np.clip(x[j], lo, hi))
                width = (hi - lo) * 0.01
                x_fallback[j] = self._sample_trunc_norm(c_valid, width, lo, hi)
            return self._clip_and_log_candidate(x_fallback, "BiasedRandom (Fallback)")
        ...
        if s2d is not None:
            ...
            if not (cond_n and (cond_pca or is_excellent) and (cond_sigma or is_excellent) and cond_r2):
                pass  # Verbose debug removed
                # self._debug_log(f"[Trial {self.trial_id}] Surrogate SKIP: n={s2d.get('n',0)}>={self.config.surrogate_min_points}({cond_n}), "  # Removed: verbose debug
        ...
        if (
            ...
            for _ in range(M_rand):
                ...
                for j in range(d):
                    ...
                    # --- FIX: Truncated Normal for Surrogate Candidates ---
                    u[j] = self._sample_trunc_norm(0.0, std, lo, hi)
                    # ----------------------------------------------------
            ...
            if M_elite > 0 and elite_centers_t.size > 0:
                ...
                for _ in range(M_elite):
                    ...
                    # map t (surrogate frame) -> original -> cube and clip
                    xg = (mu_s + R_s @ t).astype(float)
                    u = (R_c.T @ (xg - mu_c)).astype(float)
                    
                    # --- FIX: Truncated Normal for Elite Sampling ---
                    # Since we perturbed in surrogate frame, we might be out of bounds in cube frame.
                    # We can't easily use truncnorm here because the perturbation was in a rotated frame.
                    # But we can at least check and retry or clamp smartly.
                    # For now, let's just clamp, as elite sampling is aggressive by nature.
                    for j in range(d):
                        lo, hi = bounds[j]
            ...
            for u in candidates_u:
                ...
                # --- FIX: Check Global Bounds ---
                if not self._is_within_bounds(x):
                    continue
                # --------------------------------
            ...
            if not any_inside or not ei_vals:
                # self._debug_log(f"[Trial {self.trial_id}] Surrogate REJECTED: No candidates (Inside={any_inside}, EIVals={len(ei_vals)})")  # Removed: verbose debug
                pass
            if ei_vals:
                ...
                if (ei_best >= ei_min) and yhat_gate_ok:
                    # self._debug_log(f"[Trial {self.trial_id}] Sampling: Surrogate (EI={ei_best:.4f})")  # Removed: verbose debug
                    return self._clip_and_log_candidate(x_best, "Surrogate")
                else:
                    msg = f"[Trial {self.trial_id}] Surrogate REJECTED: EI={ei_best} Min={ei_min} Gate={yhat_gate_ok}"
                    # self._debug_log(msg)  # Removed: verbose debug
        ...
        # self._debug_log(f"[Trial {self.trial_id}] Sampling: Geometric Fallback (Surrogate not ready or rejected)")  # Removed: verbose debug
        
        # If very few points, prefer uniform to avoid center bias early on
        if len(cube._points_history) < 5:
             # self._debug_log(f"[Trial {self.trial_id}] Sampling: Uniform (Low History)")  # Removed: verbose debug
             ...
             for _ in range(10):
                 ...
                 if self._is_within_bounds(x):
                     return self._clip_and_log_candidate(x, "UniformFallback")
             
             # Fallback: Smart Clamp
             # self._debug_log(f"[Trial {self.trial_id}] Sampling: Uniform (Smart Fallback)")  # Removed: verbose debug
             c = cube.to_original(np.zeros(d))
             x_fallback = np.zeros(d)
             for j in range(d):
                 lo, hi = self.bounds[j]
                 c_valid = float(np.clip(c[j], lo, hi))
                 width = (hi - lo) * 0.01
                 x_fallback[j] = self._sample_trunc_norm(c_valid, width, lo, hi)
                 
             return self._clip_and_log_candidate(x_fallback, "UniformFallback")
        ...
        if np.random.rand() < self.line_search_prob:
            ...
            x = None
            for _ in range(10):
                # --- FIX: Truncated Normal for Line Search ---
                x_prime = np.zeros(d, dtype=float)
                
                # Asse principale: Uniforme (già bounded)
                lo1, hi1 = cube.bounds[0]
                if hi1 < lo1: lo1, hi1 = hi1, lo1
                x_prime[0] = np.random.uniform(lo1, hi1)
                
                # Altri assi: Truncated Normal centrata su 0
                for j in range(1, d):
                    loj, hij = cube.bounds[j]
                    if hij < loj: loj, hij = hij, loj
                    span = max(0.0, (hij - loj))
                    std = max(0.0, 0.1 * span)
                    
                    # Campiona direttamente dentro [loj, hij] centrato in 0
                    x_prime[j] = self._sample_trunc_norm(0.0, std, loj, hij)
                
                x = (mu_use + R_use @ x_prime).astype(float)
                
                if self._is_within_bounds(x):
                    # self._debug_log(f"[Trial {self.trial_id}] Sampling: Line Search (TruncNorm)")  # Removed: verbose debug
                    return self._clip_and_log_candidate(x, "LineSearch")
            
            # Fallback: Smart Clamp
            # self._debug_log(f"[Trial {self.trial_id}] Sampling: Line Search (Smart Fallback)")  # Removed: verbose debug
            x_fallback = np.zeros(d)
            for j in range(d):
                lo, hi = self.bounds[j]
                c_valid = float(np.clip(x[j], lo, hi))
                width = (hi - lo) * 0.01
                x_fallback[j] = self._sample_trunc_norm(c_valid, width, lo, hi)
            return self._clip_and_log_candidate(x_fallback, "LineSearch")
        # gaussian sampling with axis-wise std scaled by eigenvalues
        # Retry loop for global bounds
        x = None
        for _ in range(10):
            # --- FIX: Truncated Normal for Gaussian Sampling ---
            x_prime = np.zeros(d, dtype=float)
            ...
            for j in range(d):
                ...
                # Campiona direttamente dentro [lo, hi] centrato in 0
                x_prime[j] = self._sample_trunc_norm(0.0, std, lo, hi)
            
            x = (mu_use + R_use @ x_prime).astype(float)
            
            if self._is_within_bounds(x):
                # self._debug_log(f"[Trial {self.trial_id}] Sampling: Gaussian (TruncNorm)")  # Removed: verbose debug
                return self._clip_and_log_candidate(x, "Gaussian")
        
        # Fallback: Smart Clamp
        # self._debug_log(f"[Trial {self.trial_id}] Sampling: Gaussian (Smart Fallback)")  # Removed: verbose debug
        x_fallback = np.zeros(d)
        for j in range(d):
            lo, hi = self.bounds[j]
            c_valid = float(np.clip(x[j], lo, hi))
            width = (hi - lo) * 0.01
            x_fallback[j] = self._sample_trunc_norm(c_valid, width, lo, hi)
        return self._clip_and_log_candidate(x_fallback, "Gaussian")
    ...
    def _auto_tune_surrogate(self, cube: QuadCube) -> None:
        ...
        cand = int(min(512, max(128, base['candidates'] * (1.0 + 0.1 * max(0, d - 4)))))
    ...
    def select_cube(self) -> QuadCube:
        ...
        if self._preferred_leaf is not None:
            if any(c is self._preferred_leaf for c in self.leaf_cubes):
                ...
                # self._debug_log(f"[Select Cube] Using preferred leaf {id(leaf)}")  # Removed: verbose debug
                return leaf
        ...
        # self._debug_log(f"\n[Select Cube Trial {self.trial_id}] Top 5 candidates:")  # Removed: verbose debug
        for i, (c, val, b_dyn, l_dyn, r2) in enumerate(candidates[:5]):
            ...
            # self._debug_log(f"  #{i} ID={id(c)%1000} UCB={val:.4f} (Base={base:.4f}, Bonus={bonus:.4f}) "  # Removed: verbose debug
    ...
    def run_trial(self, cube: QuadCube, objective_fn: Callable[[np.ndarray, int], Any]) -> None:
        ...
        if hit_bounds:
            pass  # Verbose debug removed
            # self._debug_log(f"[Trial {self.trial_id}] Hit bounds: {', '.join(hit_bounds)} at {x_candidate}")  # Removed: verbose debug
        ...
        # Update cube stats and surrogate
        cube.update_final(float(s_final), x=x_candidate, min_points=self.config.surrogate_min_points, ridge_alpha=self.config.ridge_alpha)
        
        # --- SANITY CHECK: Log Loss ---
        self._debug_log(f"[LOSS] Trial {self.trial_id}: score={s_final_raw:.6f}, cube_best={cube.best_score:.6f}, global_best={self.sign * self.best_score_global:.6f}")
        # -------------------------------
    ...
    # No per-trial debug prints in production
        ...
        # self._debug_log(f"[Split Check Trial {self.trial_id}] Cube {id(cube)}: gamma_dyn={gamma_dyn:.6f}, gate_ratio={gate_ratio_dyn:.2f}, sigma2={cube.surrogate_2d.get('sigma2', 'N/A') if cube.surrogate_2d else 'N/A'}")  # Removed: verbose debug

        split_type = cube.should_split(
            min_trials=self.min_trials,
            min_points=self.min_trials,  # Use split threshold (2D+2) for splitting logic, not surrogate threshold (D+2)
        ...
        if split_type == 'none' and hasattr(cube, 'split_block_reason') and cube.split_block_reason:
            ...
            # self._debug_log(f"  Split Blocked: {cube.split_block_reason}")  # Removed: verbose debug
        ...
        if split_type != 'none':
            ...
            if children:
                ...
                # Log split event
                # self._debug_log(f"\n[SPLIT at Trial {self.trial_id}] Cube {id(cube)} (depth={cube.depth}) split into {len(children)} children")  # Removed: verbose debug
                # self._debug_log(f"  Parent: n_trials={cube.n_trials}, best={cube.best_score:.4f}")  # Removed: verbose debug
                # for i, child in enumerate(children):  # Removed: verbose debug
                #     self._debug_log(f"  Child {i}: depth={child.depth}, inherited_history={len(child._points_history)}")  # Removed: verbose debug
        ...
        if removed > 0:
            pass  # Verbose debug removed
            # self._debug_log(f"\n[PRUNE at Trial {self.trial_id}] Removed {removed} cube(s), {len(self.leaf_cubes)} remain")  # Removed: verbose debug
    ...
    def prune_cubes(self) -> None:
        ...
        delta_max = 1.0
        # Relaxed to linear decay to avoid over-pruning in mid-game
        delta_dyn = delta_max * (1.0 - progress)
        ...
        # self._debug_log(f"[Prune Check Trial {self.trial_id}] Best={best_signed:.4f}, Delta={delta_dyn:.4f}, Thresh={keep_thresh:.4f}, Progress={progress:.2f}")  # Removed: verbose debug
        ...
        for c in ranked:
            ...
            if not ok:
                pass  # Verbose debug removed
                # self._debug_log(f"  Pruning Cube {id(c)%1000}: UCB={val:.4f} < Thresh={keep_thresh:.4f}")  # Removed: verbose debug

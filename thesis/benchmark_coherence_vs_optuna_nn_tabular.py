#!/usr/bin/env python3
"""Benchmark: ALBA Coherence vs Optuna TPE on HPOBench NN Tabular benchmarks.

Runs on all locally available HPOBench TabularBenchmark datasets for model='nn'.
Budget=400, multiple seeds.

Example:
  python thesis/benchmark_coherence_vs_optuna_nn_tabular.py --budget 400 --seeds 0-9
"""
...
def _available_task_ids(model: str = "nn") -> List[int]:
...
def main() -> int:
    p = argparse.ArgumentParser(description="Coherence vs Optuna on NN Tabular")
    ...
    if args.task_ids.strip():
        ...
        task_ids = _available_task_ids(model="nn")
    ...
    out_path = f"{RESULTS_DIR}/coherence_vs_optuna_nn_tabular_b{args.budget}_s{seeds[0]}-{seeds[-1]}_{timestamp}.json"
    ...
    for task_id in task_ids:
        benchmark = TabularBenchmark(model="nn", task_id=int(task_id), rng=42)

...
from .leaf_selection import LeafSelector, UCBSoftmaxLeafSelector, PotentialAwareLeafSelector, ThompsonSamplingLeafSelector
...
class ALBA:
    ...
    def __init__(
        ...
        self.drilling_budget_used = 0  # Track total iterations spent drilling
        self.drilling_budget_max = int(total_budget * 0.4)  # Max 40% of budget for drilling
        ...
        # Use ThompsonSamplingLeafSelector by default (won empirically vs Base)
        self._leaf_selector: LeafSelector = (
            leaf_selector 
            if leaf_selector is not None 
            else ThompsonSamplingLeafSelector()
        )
    ...
    def ask_array(self) -> np.ndarray:
        ...
        if self.driller is not None:
            ...
            if x is not None:
                # BUG FIX: Set _last_cube so tell() can record the observation!
    ...
    def tell(self, x: Union[np.ndarray, Dict[str, Any]], y_raw: float) -> None:
        ...
        if self.driller is not None:
            ...
            # So pass -y as cost.
            self.drilling_budget_used += 1
            continue_drilling = self.driller.tell(x, -y)
            # Also check global drilling budget cap
            if not continue_drilling or self.drilling_budget_used >= self.drilling_budget_max:
        ...
        # But only after warmup period and if we haven't exhausted drilling budget
        can_start_drilling = (
            self.drilling_enabled
            and new_best_found
            and self.driller is None
            and self.iteration >= self.drilling_warmup
            and self.drilling_budget_used < self.drilling_budget_max
        )
        if can_start_drilling:
    ...
    def _sample_in_cube(self, cube: Cube) -> np.ndarray:
        ...
        if self.iteration < 15:
            ...
            if self._coherence_tracker is not None and self._use_potential_field:
                raw_potential = self._coherence_tracker.get_potential(cube, self.leaves)
                # STABILITY FIX: Scale potential influence by global coherence
                # When coherence is low (~0.5), potential is unreliable -> use neutral 0.5
                global_coh = self._coherence_tracker.global_coherence
                coherence_scale = max(0.0, min(1.0, (global_coh - 0.5) * 3.33))
                # Blend between neutral (0.5) and actual potential
                potential = 0.5 + coherence_scale * (raw_potential - 0.5)

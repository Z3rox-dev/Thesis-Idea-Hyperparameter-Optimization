...
def run_alba(func, bounds, budget, drilling=False):
    ...
    optimizer = ALBA(
        ...
        seed=np.random.randint(100000)  # Random seed per ripetizione
    )
...
def run_alba_covariance(func, bounds, budget, drilling=False):
    ...
    # Inject weighted covariance sampler (uses default parameters now - improved version)
...
def run_all(test_run=False):
    ...
    if test_run:
        ...
        tasks_to_run = [
            # 10D - Higher dimensionality
            {"name": "Sphere", "dim": 10, "budget": 300},
            {"name": "Rastrigin", "dim": 10, "budget": 300},
            {"name": "Rosenbrock", "dim": 10, "budget": 300},
            {"name": "Ellipsoid", "dim": 10, "budget": 300},
            {"name": "Cigar", "dim": 10, "budget": 300},
            {"name": "Discus", "dim": 10, "budget": 300},
            # 20D - High dimensionality
    ...
    if not test_run:
        ...
        print("="*80)
        # Create combined index for Function+Dim to handle multiple dimensions
        df['Task'] = df['Function'] + '_' + df['Dim'].astype(str) + 'D'
        print(df.pivot(index='Task', columns='Optimizer', values='Mean').to_string())

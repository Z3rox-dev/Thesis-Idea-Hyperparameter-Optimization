...
class Cube:
    ...
    def get_split_axis(self) -> int:
        ...
        Selection strategy designed to balance exploration across all dimensions
        while still leveraging gradient information when reliable.
        ...
        min_width = widths.min() + 1e-9
        
        # Compute aspect ratio
        aspect_ratio = max_width / min_width
        widest_axis = int(np.argmax(widths))

        # RULE 1: If cube is elongated (aspect > 2), ALWAYS split the widest
        # This ensures cubes stay reasonably balanced
        if aspect_ratio > 2.0:
            return widest_axis

        # RULE 2: Early splits (depth <= 2) - prefer widest dimension
        # This prevents early noise from biasing all future splits
        if self.depth <= 2:
            return widest_axis

        # RULE 3: For deeper, balanced cubes - use gradient if reliable
        # Gradient threshold: must be significantly stronger than uniform
        uniform_component = 1.0 / np.sqrt(dim)
        gradient_threshold = uniform_component * 2.0  # ~0.89 for dim=5 (very selective)

        if self.lgs_model is not None and self.lgs_model["gradient_dir"] is not None:
            grad_dir = np.abs(self.lgs_model["gradient_dir"])
            
            if grad_dir.max() > gradient_threshold:
                grad_axis = int(np.argmax(grad_dir))
                
                # Gradient axis must still be reasonably wide
                if widths[grad_axis] >= 0.5 * max_width:
                    return grad_axis

        # RULE 4: Variance of good points
        good_pts = np.array(
            [p for p, s in self._tested_pairs if s >= self.best_score * 0.95]
        )
        if len(good_pts) >= 3:
            var_per_dim = np.var(good_pts / (widths + 1e-9), axis=0)
            score = var_per_dim * (widths / (max_width + 1e-9))
            if score.max() > 0.01:

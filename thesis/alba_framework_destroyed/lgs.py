...
def fit_lgs_model(
    ...
    if len(pairs) >= dim + 3:
        ...
        try:
            ...
            y_mean_w = float(np.average(y_centered, weights=weights))
            sst = float(np.average((y_centered - y_mean_w) ** 2, weights=weights))
            if sst > 1e-12:
                r2 = float(1.0 - float(np.average(residuals**2, weights=weights)) / sst)

...
def compute_coherence_scores(
    ...
    # Step 5: Solve global potential field
    # We use 1.0 + alignment as weight (higher alignment = stronger edge)
    weights = np.array(alignments) + 1.0 
    u = _solve_potential_least_squares(n, valid_edges, d_lm, weights)
    
    # Step 5b: Re-anchor potential based on observed performance
    # Leaves with better performance should have LOWER potential (more promising)
    leaf_good_ratios = np.array([leaves[i].good_ratio() for i in range(n)])
    
    # Invert u (gradient potential says "where gradients converge", but we want "where minimum is")
    # For minimization: gradients point AWAY from minimum, so convergence point is actually the peak
    u_inverted = -u
    
    # Combine with empirical signal: higher good_ratio = lower potential (better)
    empirical_bonus = leaf_good_ratios * 2.0  # Scale to make it significant
    u_combined = u_inverted - empirical_bonus
    
    # Re-anchor so best leaf (highest good_ratio) has potential 0
    best_leaf_idx = int(np.argmax(leaf_good_ratios))
    u_anchored = u_combined - u_combined[best_leaf_idx]
    
    # Check if potential field is informative
    # In high dimensions with few edges, variance can be too low to be useful
    u_var = np.var(u_anchored) if u_anchored.size > 0 else 0.0
    MIN_POTENTIAL_VARIANCE = 0.001
    
    if u_var < MIN_POTENTIAL_VARIANCE:
        # Potential is uninformative, return neutral values
        # But still use good_ratio as a proxy: lower ratio = higher potential
        # This provides some signal even when graph is sparse
        if np.std(leaf_good_ratios) > 0.01:
            # Use good_ratio directly: high ratio = low potential (good)
            u_norm = 1.0 - leaf_good_ratios  # Invert: high ratio → low potential
        else:
            u_norm = np.full(n, 0.5)  # Truly neutral
    else:
        # Robust Min-Max normalization
        # We want the Best Leaf to have potential ~0.0 (Blue)
        # And the Worst Leaf to have potential ~1.0 (Red)

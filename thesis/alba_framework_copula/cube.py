"""
ALBA Framework (Copula variant) - Cube Module

This module implements the Cube class, representing a hyperrectangle region
of the search space with a local Copula model (instead of LGS).
...
# HYBRID: Use LGS for prediction (robust) + copula for SAMPLING only
from alba_framework_potential.lgs import fit_lgs_model as _fit_lgs_internal
from alba_framework_potential.lgs import predict_bayesian as _predict_bayesian
from .copula_model import fit_copula_model as _fit_copula_internal

def _fit_lgs_model(cube, gamma, dim, rng=None):
    """Hybrid: fit both LGS and copula, merge results.
    
    - LGS provides: grad, inv_cov for prediction
    - Copula provides: L, marginal_params for sampling
    """
    lgs_model = _fit_lgs_internal(cube, gamma, dim, rng)
    copula_model = _fit_copula_internal(cube, gamma, dim, rng)
    
    if lgs_model is None:
        return copula_model  # Fallback
    
    # Add bounds from cube
    lgs_model["bounds"] = cube.bounds
    
    if copula_model is not None:
        # Merge: keep LGS for prediction, add copula for sampling
        lgs_model["L"] = copula_model.get("L")
        lgs_model["marginal_params"] = copula_model.get("marginal_params")
        lgs_model["corr"] = copula_model.get("corr")
        lgs_model["elite_pts"] = copula_model.get("elite_pts")
        lgs_model["elite_scores"] = copula_model.get("elite_scores")
...
class Cube:
    ...
    def get_split_axis(self) -> int:
        ...
        Selection priority:
        1. Correlation importance from copula model
        ...
        # Primary: correlation-based (use dimension with highest correlation variance in copula)
        if self.lgs_model is not None and self.lgs_model.get("corr") is not None:
            corr = self.lgs_model["corr"]
            # Use off-diagonal sum as measure of correlation importance
            corr_importance = np.abs(corr).sum(axis=1) - 1.0  # Subtract diagonal
            corr_importance = corr_importance * (widths / (widths.max() + 1e-9))
            if corr_importance.max() > 0.1:
                return int(np.argmax(corr_importance))

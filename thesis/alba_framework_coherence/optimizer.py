...
class ALBA:
    ...
    def _sample_in_cube(self, cube: Cube) -> np.ndarray:
        ...
        Uses coherence gating when enabled:
        - High coherence (> Q60): exploit with gradient-based LGS sampling
        - Low coherence (<= Q60): explore with random/space-filling sampling
        ...
        if self.iteration < 15:
            ...
            # Coherence-gated exploit/explore
            use_exploit = True
            if self._coherence_tracker is not None and self._use_coherence_gating:
                use_exploit = self._coherence_tracker.should_exploit(cube, self.leaves)
            
            if use_exploit:
                # Trust gradient - use LGS-based acquisition
                x = self._sample_with_lgs(cube)
            else:
                # Don't trust gradient - use space-filling exploration

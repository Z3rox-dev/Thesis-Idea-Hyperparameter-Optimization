...
class ECFS:
    ...
    def ask(self) -> np.ndarray:
        ...
        if self.cfg.use_ratio:
            ...
            # No-ratio ablation: do not build a contrast model.
            model_N, cond_N = None, float("nan")
            fallback_N = True
        ...
        # Score by contrastive ratio. For the no-ratio ablation, keep pure sampling from p_E
        # (do NOT re-rank by logp_E, which would bias towards tiny/typical steps).
        if self.cfg.use_ratio:
            logp_E = model_E.logpdf(deltas)
            logp_N = model_N.logpdf(deltas) if model_N is not None else 0.0
            score = logp_E - logp_N
            order = np.argsort(score)[::-1]
            batch_top_score = float(score[order[0]]) if order.size else float("nan")
        else:
            score = None
            order = np.arange(int(self.cfg.M))
            batch_top_score = float("nan")
        ...
        self._record_ask_meta(
            ...
            cond_elite=cond_E,
            cond_nonelite=cond_N,
            batch_top_score=batch_top_score,
            from_pending=False,
        )
    ...
    def _fit_gaussian(self, deltas: np.ndarray) -> tuple["_GaussianModel", float]:
        ...
        # Always estimate covariance on centered deltas; mu_zero should only remove drift.
